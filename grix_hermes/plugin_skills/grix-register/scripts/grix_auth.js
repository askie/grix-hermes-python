...
class GrixAuthError extends Error {
    ...
    code;
    payload;
    constructor(message, status = 0, code = -1, payload = null) {
        super(message);
        this.status = status;
        this.code = code;
...
function derivePortalUrl(rawBaseUrl) {
    const base = cleanText(rawBaseUrl) || resolveDefaultBaseUrl();
    let parsed;
    try {
        parsed = new URL(base);
...
async function requestJson(method, pathPart, baseUrl, body, headers) {
...
function parseOptionalBool(value, defaultValue) {
...
function buildAuthResult(action, result, baseUrl) {
...
function buildAgentResult(action, result, isMain) {
    const data = (result.data && typeof result.data === "object" ? result.data : {});
...
async function loginWithCredentials(baseUrl, account, password, deviceId, platform) {
...
async function createApiAgent(baseUrl, accessToken, agentName, avatarUrl, isMain) {
    const requestBody = {
...
async function listAgents(baseUrl, accessToken) {
...
async function rotateApiAgentKey(baseUrl, accessToken, agentId, isMain) {
    const result = await requestJson("POST", `/agents/${agentId.trim()}/api/key/rotate`, baseUrl, {}, { Authorization: `Bearer ${accessToken.trim()}` });
    return buildAgentResult("rotate-api-agent-key", result, Boolean(isMain));
}
function findExistingApiAgent(agents, agentName) {
    ...
    for (const item of agents) {
        if (!item || typeof item !== "object")
            continue;
        if (cleanText(item.agent_name) !== normalizedName)
            continue;
        if (Number.parseInt(String(item.provider_type ?? 0), 10) !== 3)
            continue;
        if (Number.parseInt(String(item.status ?? 0), 10) === 3)
            continue;
        return item;
    }
    return null;
}
async function createOrReuseApiAgent(baseUrl, accessToken, agentName, avatarUrl, preferExisting, rotateOnReuse, isMain) {
    if (preferExisting) {
        const agents = await listAgents(baseUrl, accessToken);
        const existing = findExistingApiAgent(agents, agentName);
        if (existing) {
            if (!rotateOnReuse) {
                throw new GrixAuthError("existing provider_type=3 agent found but rotate-on-reuse is disabled; cannot obtain api_key safely", 0, -1, { existing_agent: existing });
            }
            const rotated = await rotateApiAgentKey(baseUrl, accessToken, cleanText(existing.id), parseOptionalBool(existing.is_main, Boolean(isMain)));
...
async function handleSendEmailCode(baseUrl, flags) {
...
async function handleRegister(baseUrl, flags) {
...
async function handleLogin(baseUrl, flags) {
    ...
    const platform = cleanText(flagString(flags, "platform", "web")) || "web";
    const deviceId = cleanText(flagString(flags, "device_id")) || defaultDeviceId(platform);
    const result = await loginWithCredentials(baseUrl, account, cleanText(flagRequired(flags, "password")), deviceId, platform);
    printJson(result);
}
async function handleCreateApiAgent(baseUrl, flags) {
    const requestedIsMain = parseOptionalBool(flagString(flags, "is_main"), true);
    const result = await createOrReuseApiAgent(baseUrl, cleanText(flagRequired(flags, "access_token")), cleanText(flagRequired(flags, "agent_name")), flagString(flags, "avatar_url"), !Boolean(flags.no_reuse_existing_agent), !Boolean(flags.no_rotate_key_on_reuse), Boolean(requestedIsMain));
    printJson(result);
}
async function main() {
    let parsed;
