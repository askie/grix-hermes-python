...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (token === "--agent-json-file") {
            const [v, j] = take(i);
            flags.agentJsonFile = v;
            i = j;
            continue;
        }
...
function loadOrCreatePayload(flags, authScript) {
    if (flags.agentJsonFile) {
        return JSON.parse(fs.readFileSync(flags.agentJsonFile, "utf8"));
    }
    ...
    if (flags.avatarUrl)
        cmd.push("--avatar-url", flags.avatarUrl);
...
function main() {
    ...
    try {
        ...
        if (bindIsMain)
            cmd.push("--is-main", bindIsMain);
        if (flags.profileName)
            cmd.push("--profile-name", flags.profileName);
        if (flags.cloneFrom)
            cmd.push("--clone-from", flags.cloneFrom);
        if (flags.installDir)
            cmd.push("--install-dir", flags.installDir);
        if (flags.accountId)
            cmd.push("--account-id", flags.accountId);
        if (flags.allowedUsers)
            cmd.push("--allowed-users", flags.allowedUsers);
        if (flags.allowAllUsers)
            cmd.push("--allow-all-users", flags.allowAllUsers);
        if (flags.homeChannel)
            cmd.push("--home-channel", flags.homeChannel);
        if (flags.homeChannelName)
            cmd.push("--home-channel-name", flags.homeChannelName);
        if (flags.inheritKeys)
            cmd.push("--inherit-keys", flags.inheritKeys);
        if (flags.dryRun)
            cmd.push("--dry-run");
        if (flags.json)
            cmd.push("--json");
