...
import { formatWsCredentialDiagnostics, getWsCredentialDiagnostics, hasWsCredentials, } from "../../shared/cli/config.js";
...
function resolveProfileRoot(hermesHome) {
...
function slugifyProfileCandidate(value) {
...
function resolveProfileNameSuffix(value) {
    const normalized = slugifyProfileCandidate(value);
    return normalized === "default" ? "" : normalized;
}
function buildProfileNameWithSuffix(profileNameSuffix) {
...
function resolveBootstrapProfileName(explicitProfileName, agentName, profileNameSuffix) {
    const requested = cleanText(explicitProfileName);
    if (requested)
        return requested;
    const requestedAgentName = cleanText(agentName);
    if (isValidProfileName(requestedAgentName))
...
function validateProfileNameSuffix(profileNameSuffix) {
    const requested = cleanText(profileNameSuffix);
    if (!requested)
        return;
    const normalized = resolveProfileNameSuffix(requested);
    if (!normalized) {
        throw new Error("Invalid --profile-name-suffix: must contain at least one ASCII letter or number after normalization.");
    }
}
// ---------------------------------------------------------------------------
// Section 2: Types
// ---------------------------------------------------------------------------
const STEP_NAMES = ["detect", "install", "create", "bind", "soul", "gateway", "accept"];
function setStatePath(state, pathUsed) {
    state.path = pathUsed;
}
class BootstrapError extends Error {
    ...
    suggestion;
    rawError;
    constructor(step, stepNumber, reason, suggestion, rawError) {
        super(reason);
...
function redactStateValue(value) {
    if (Array.isArray(value))
...
function resolveHomeChannelName(flags, installContext) {
...
function resolveHomeChannel(flags, installContext) {
...
function resolveDeliveryTarget(flags, installContext) {
    const statusTarget = cleanText(flags.statusTarget);
    if (statusTarget)
        return { target: statusTarget, source: "status_target" };
    const homeChannel = cleanText(flags.homeChannel);
    if (homeChannel)
        return { target: homeChannel, source: "home_channel" };
    const currentChatSessionId = cleanText(installContext?.current_chat_session_id);
    if (currentChatSessionId) {
...
// ---------------------------------------------------------------------------
function getStateFilePath(hermesHome, installId) {
...
function normalizeDeliveryTargetSource(value) {
    const normalized = cleanText(value);
    if (normalized === "status_target" ||
...
function refreshInteractionStatus(state) {
    ...
    const hasSuccessFinal = ["final_text", "final_card"].every((kind) => state.delivery.attempts.some((attempt) => attempt.kind === kind && attempt.ok));
    const hasFailureFinal = ["failure_text", "failure_card"].every((kind) => state.delivery.attempts.some((attempt) => attempt.kind === kind && attempt.ok));
    state.interaction_status = hasSuccessFinal || hasFailureFinal ? "full" : "none";
}
function setResolvedDeliveryTarget(state, target) {
    ...
    const attempts = Array.isArray(record.attempts)
        ? record.attempts.flatMap((item) => {
            if (!item || typeof item !== "object" || Array.isArray(item))
                return [];
            const attempt = item;
            const kind = cleanText(attempt.kind);
            const allowedKinds = [
                "running_card",
                "acceptance_card",
                "final_text",
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i += 1) {
        const token = argv[i];
        const next = argv[i + 1];
        if (token === "--install-id" && next !== undefined) {
            flags.installId = next;
            i += 1;
            continue;
        }
        if (token === "--install-context" && next !== undefined) {
            flags.installContext = next;
            i += 1;
            continue;
        }
        if (token === "--agent-name" && next !== undefined) {
            flags.agentName = next;
            i += 1;
            continue;
        }
        if (token === "--profile-name-suffix" && next !== undefined) {
            flags.profileNameSuffix = next;
            i += 1;
            continue;
        }
        if (token === "--status-target" && next !== undefined) {
            flags.statusTarget = next;
            i += 1;
            continue;
        }
        if (token === "--soul-content" && next !== undefined) {
            flags.soulContent = next;
            i += 1;
            continue;
        }
        if (token === "--soul-file" && next !== undefined) {
            flags.soulFile = next;
            i += 1;
            continue;
        }
        if (token === "--access-token" && next !== undefined) {
            flags.accessToken = next;
            i += 1;
            continue;
        }
        if (token === "--account" && next !== undefined) {
            flags.account = next;
            i += 1;
            continue;
        }
        if (token === "--email" && next !== undefined) {
            flags.email = next;
            i += 1;
            continue;
        }
        if (token === "--password" && next !== undefined) {
            flags.password = next;
            i += 1;
            continue;
        }
        if (token === "--base-url" && next !== undefined) {
            flags.baseUrl = next;
            i += 1;
            continue;
        }
        if (token === "--avatar-url" && next !== undefined) {
            flags.avatarUrl = next;
            i += 1;
            continue;
        }
        if (token === "--category-name" && next !== undefined) {
            flags.categoryName = next;
            i += 1;
            continue;
        }
        if (token === "--is-main" && next !== undefined) {
            flags.isMain = next;
            i += 1;
            continue;
        }
        if (token === "--route" && next !== undefined) {
            flags.route = next;
            i += 1;
            continue;
        }
        ...
        if (token === "--probe-message" && next !== undefined) {
            flags.probeMessage = next;
            i += 1;
            continue;
        }
        if (token === "--expected-substring" && next !== undefined) {
            flags.expectedSubstring = next;
            i += 1;
            continue;
        }
        if (token === "--member-ids" && next !== undefined) {
            flags.memberIds = next;
            i += 1;
            continue;
        }
        if (token === "--member-types" && next !== undefined) {
            flags.memberTypes = next;
            i += 1;
            continue;
        }
        if (token === "--agent-id" && next !== undefined) {
            flags.agentId = next;
            i += 1;
            continue;
        }
        if (token === "--api-endpoint" && next !== undefined) {
            flags.apiEndpoint = next;
            i += 1;
            continue;
        }
        if (token === "--api-key" && next !== undefined) {
            flags.apiKey = next;
            i += 1;
            continue;
        }
        if (token === "--bind-json" && next !== undefined) {
            flags.bindJson = next;
            i += 1;
            continue;
        }
        if (token === "--account-id" && next !== undefined) {
            flags.accountId = next;
            i += 1;
            continue;
        }
        if (token === "--allowed-users" && next !== undefined) {
            flags.allowedUsers = next;
            i += 1;
            continue;
        }
        if (token === "--allow-all-users" && next !== undefined) {
            flags.allowAllUsers = next;
            i += 1;
            continue;
        }
        if (token === "--home-channel" && next !== undefined) {
            flags.homeChannel = next;
            i += 1;
            continue;
        }
        if (token === "--home-channel-name" && next !== undefined) {
            flags.homeChannelName = next;
            i += 1;
            continue;
        }
        if (token === "--accept-timeout-seconds" && next !== undefined) {
            flags.acceptTimeoutSeconds = next;
            i += 1;
            continue;
        }
        if (token === "--accept-poll-interval-seconds" && next !== undefined) {
            flags.acceptPollIntervalSeconds = next;
            i += 1;
            continue;
        }
        if (token === "--resume") {
...
// ---------------------------------------------------------------------------
function suggestForError(step, errorMessage) {
...
// ---------------------------------------------------------------------------
function stepDetect(flags, state, _scripts, hermesHome) {
    if (stepIsDone(state, "detect"))
        return;
    const route = cleanText(flags.route) || "create_new";
    if (route !== "create_new" && route !== "existing") {
        throw new BootstrapError("detect", 1, `不支持的 route: ${route}`, "使用 --route create_new 创建新 agent，或 --route existing 绑定已有 agent 凭证。", `unsupported route=${route}`);
    }
    if (route === "existing") {
        if (!cleanText(flags.bindJson) && (!cleanText(flags.agentId) || !cleanText(flags.apiEndpoint) || !cleanText(flags.apiKey))) {
            throw new BootstrapError("detect", 1, "--route existing 需要提供 --agent-id、--api-endpoint、--api-key，或提供 --bind-json", "已有 agent 绑定必须显式提供完整凭证；不要从 checkpoint 或终端脱敏输出里恢复 API key。", "missing existing bind credentials");
        }
    ...
    throw new BootstrapError("detect", 1, "未检测到可复用的 Grix 宿主会话凭证，且也没有可用于 HTTP fallback 的 access token / 登录账号信息", "请先检查当前 Hermes 会话是否已连接 Grix；如果宿主链路不可用，则向用户获取邮箱/账号和密码，通过 grix-register login/register 换取 access token 后再重试；或显式提供已有 agent 凭证走 --route existing。", formatWsCredentialDiagnostics(wsDiagnostics));
}
...
function normalizeBindPayload(payload, flags) {
...
function requireExistingCredentials(flags) {
...
function stepInstall(flags, state, scripts, hermesHome, env) {
    if (stepIsDone(state, "install"))
        return;
...
async function stepCreate(flags, state, scripts, hermesHome, env) {
    if (stepIsDone(state, "create")) {
        return {
            credentials: credentialsFromFlags(flags),
            pathUsed: cleanText(state.steps.create?.result?.["path"]),
        };
    }
...
async function stepCreateWs(flags, state, scripts, env) {
    ...
    if (!agentId || !apiEndpoint || !apiKey) {
        throw new BootstrapError("create", 3, `WS 创建 agent 未返回有效凭证。agent_id=${agentId}, api_endpoint=${apiEndpoint}, api_key=${apiKey ? "ak_***" : ""}`, "检查 WS 连接和 GRIX_API_KEY 是否有效。确认 agent_api_create 接口正常。", result.stderr || result.stdout);
    }
    markStepDone(state, "create", {
        path: "host",
        agent_id: agentId,
...
function stepCreateExisting(flags, state) {
...
function stepCreateHttp(flags, state, scripts, hermesHome, env) {
    const installDir = cleanText(flags.installDir) || defaultInstallDir(hermesHome);
    ...
    if (!accessToken) {
        ...
        if (!account || !password) {
            throw new BootstrapError("create", 3, "HTTP fallback 缺少 access token，且也没有可用于登录的账号/邮箱和密码", "请先向用户获取邮箱/账号和密码，通过 grix-register login 换取 access token；如用户还没有账号，则先走 send-email-code/register/login，再重试。", `missing HTTP auth inputs: access_token=${accessToken ? "present" : "missing"}, account=${account ? "present" : "missing"}, password=${password ? "present" : "missing"}`);
        }
        const loginCmd = [
            flags.node,
        ...
        if (cleanText(flags.email))
            loginCmd.push("--email", cleanText(flags.email));
        else
            loginCmd.push("--account", cleanText(flags.account));
        ...
        if (!accessToken) {
            throw new BootstrapError("create", 3, "grix-register login 未返回 access token，无法继续 HTTP fallback 创建 agent", "检查邮箱/账号密码是否正确，并确认 grix-register login 对当前 Grix 环境可用。", loginResult.stderr || loginResult.stdout);
        }
    }
    const cmd = [
        flags.node,
    ...
    if (!agentId || !apiEndpoint) {
        throw new BootstrapError("create", 3, `HTTP 创建 agent 未返回有效凭证。agent_id=${agentId}, api_endpoint=${apiEndpoint}`, "检查是否已先通过 grix-register login/register 获取有效 access token，并确认网络可访问 Grix API。", result.stderr || result.stdout);
    }
    markStepDone(state, "create", {
        path: "http",
        agent_id: agentId,
...
function stepBind(flags, state, scripts, hermesHome, env, credentials) {
    if (stepIsDone(state, "bind"))
        return;
    const profileRoot = resolveProfileRoot(hermesHome);
    const createResult = state.steps.create?.result;
    const pathUsed = state.path || state.steps.create?.result?.["path"] || state.steps.detect?.result?.["path"];
    if (pathUsed === "http") {
    ...
    if (!agentId || !apiEndpoint || !apiKey) {
        throw new BootstrapError("bind", 4, "绑定需要 agent_id、api_endpoint 和未脱敏 api_key", "如果是在 create 后中断再 --resume，请改用 --route existing 并显式提供 --agent-id、--api-endpoint、--api-key 或 --bind-json。", `agent_id=${agentId}, api_endpoint=${apiEndpoint}, api_key=${apiKey ? "ak_***" : ""}`);
    }
...
function stepSoul(flags, state, _scripts, hermesHome) {
    if (stepIsDone(state, "soul"))
        return;
    ...
    if (soulFile) {
        if (!fs.existsSync(soulFile)) {
            throw new BootstrapError("soul", 5, `SOUL 文件不存在: ${soulFile}`, "检查 --soul-file 路径是否正确。", `file not found: ${soulFile}`);
        }
...
function stepGateway(flags, state, scripts, hermesHome, env) {
    if (stepIsDone(state, "gateway"))
        return;
    const profileName = state.profile_name || flags.profileName || flags.agentName;
    const cmd = [
...
async function stepAccept(flags, state, scripts, deliveryTarget, hermesHome, env) {
    if (stepIsDone(state, "accept"))
        return;
    ...
    if (!targetAgentId) {
        throw new BootstrapError("accept", 7, "验收需要目标 agent_id，但 create 步骤没有记录 agent_id", "检查 create 步骤是否成功；已有 agent 路径请提供 --agent-id。", "missing target agent_id");
    }
    ...
    if (!acceptanceSessionId) {
        throw new BootstrapError("accept", 7, `测试群创建未返回 session_id: ${JSON.stringify(groupData)}`, "检查 Grix WS 连接是否正常，session 是否有效。", groupResult.stderr || groupResult.stdout);
    }
    if (deliveryTarget.target) {
        ...
        sendDeliveryMessage(flags, state, scripts, deliveryTarget, "acceptance_card", cardText, hermesHome);
    }
    const acceptanceProbeMessage = buildAcceptanceProbeMessage(targetAgentId, probeMessage);
    const probeSentAtMs = Date.now();
    const probeSendResult = runCommand([flags.node, scripts.sendScript, "--to", acceptanceSessionId, "--message", acceptanceProbeMessage], { env: cmdEnv });
    ...
    while (Date.now() < deadline) {
        ...
        const verifiedMessage = messages.find((message) => messageMatchesAcceptance({
            message,
            targetAgentId,
            expectedLower,
    ...
    throw new BootstrapError("accept", 7, requireExpectedSubstring
        ? `验收超时：agent 未在 ${timeoutSeconds} 秒内回复包含「${expectedSubstring}」的内容`
        : `验收超时：agent 未在 ${timeoutSeconds} 秒内给出 probe 之后的首条非空回复`, "检查：(1) SOUL.md 内容是否正确，(2) 网关是否在线（hermes --profile <name> gateway status），(3) agent 是否已连接到 Grix。", `target_agent_id=${targetAgentId}, probe_message_id=${probeMessageId}, candidates=${lastCandidateCount}, last_query=${JSON.stringify(lastQuery)}`);
}
// ---------------------------------------------------------------------------
// Section 9: Helpers
// ---------------------------------------------------------------------------
function buildAcceptanceProbeMessage(targetAgentId, probeMessage) {
...
function extractNested(payload, key) {
...
function buildAcceptanceMembers(targetAgentId, memberIdsText, memberTypesText) {
    ...
    if (memberTypes.length > 0 && memberTypes.length !== memberIds.length) {
        throw new BootstrapError("accept", 7, "--member-types 数量必须和 --member-ids 一致", "批量验收成员请同时提供一一对应的 --member-ids 和 --member-types；不提供 member-types 时默认用户类型为 1。", `member_ids=${memberIds.length}, member_types=${memberTypes.length}`);
    }
...
function validateProfileName(profileName) {
    if (!isValidProfileName(profileName)) {
        throw new Error(`Invalid Hermes profile name: ${profileName}. Must match [a-z0-9][a-z0-9_-]{0,63}`);
    }
}
...
function collectMessageRecords(value, results, depth) {
    if (depth > 8 || !value)
        return;
    if (Array.isArray(value)) {
...
function looksLikeMessageRecord(record) {
    return Boolean(extractMessageText(record) ||
...
function extractSenderIds(record) {
    const ids = [];
    const push = (value) => {
        const text = cleanText(value);
...
function extractMessageTimeMs(record) {
...
function messageMatchesAcceptance(params) {
    ...
    if (!text)
        return false;
    if (params.requireExpectedSubstring && !text.toLowerCase().includes(params.expectedLower))
        return false;
    if (!extractSenderIds(params.message).includes(params.targetAgentId))
        return false;
    const messageId = parseNumericId(extractMessageId(params.message));
    const probeId = parseNumericId(params.probeMessageId);
    if (messageId !== null && probeId !== null)
        return messageId > probeId;
    const messageTime = extractMessageTimeMs(params.message);
    if (messageTime !== null)
        return messageTime >= params.probeSentAtMs - 1000;
    return false;
}
function wasDeliveryAttemptSuccessful(state, kind) {
    return state.delivery.attempts.some((attempt) => attempt.kind === kind && attempt.ok);
}
function buildSuccessSummary(state) {
...
function buildFailureSummary(agentName, step, reason, suggestion) {
...
function buildSuccessOutput(state, backupDir) {
    const acceptResult = state.steps.accept?.result || {};
    ...
    const output = {
        ...
        acceptance,
        steps: Object.fromEntries(STEP_NAMES.map((name) => [name, { status: state.steps[name].status }])),
    };
...
function recordDeliveryAttempt(state, deliveryTarget, kind, message, result) {
    if (!deliveryTarget.target)
        return;
...
function sendDeliveryMessage(flags, state, scripts, deliveryTarget, kind, message, hermesHome) {
    if (!deliveryTarget.target)
        return;
    ...
    try {
        const result = runCommand([flags.node, scripts.sendScript, "--to", deliveryTarget.target, "--message", message], { env, check: false });
        if (result.code === 0) {
            const payload = parseJsonOutput(result);
...
function sendStatusCard(flags, state, scripts, deliveryTarget, kind, status, step, summary, hermesHome) {
...
// ---------------------------------------------------------------------------
function buildResumeCommand(flags) {
    const parts = ["node", "scripts/bootstrap.js"];
    parts.push("--install-id", flags.installId);
    parts.push("--agent-name", flags.agentName);
    if (flags.installContext && flags.installContext !== "-")
        parts.push("--install-context", flags.installContext);
    if (flags.route)
        parts.push("--route", flags.route);
    if (flags.profileName)
        parts.push("--profile-name", flags.profileName);
    else if (flags.profileNameSuffix)
        parts.push("--profile-name-suffix", flags.profileNameSuffix);
    if (flags.soulFile)
        parts.push("--soul-file", flags.soulFile);
    if (flags.soulContent)
        parts.push("--soul-content", `'${flags.soulContent.slice(0, 30)}...'`);
    if (flags.statusTarget)
        parts.push("--status-target", flags.statusTarget);
    if (flags.homeChannel)
        parts.push("--home-channel", flags.homeChannel);
    if (flags.homeChannelName)
        parts.push("--home-channel-name", flags.homeChannelName);
    if (flags.expectedSubstring)
        parts.push("--expected-substring", flags.expectedSubstring);
    if (flags.bindJson && flags.bindJson !== "-")
        parts.push("--bind-json", flags.bindJson);
...
async function main() {
    ...
    if (!requestedProfileName && !isValidProfileName(cleanText(flags.agentName)) && !cleanText(flags.profileNameSuffix)) {
        process.stderr.write("Non-ASCII or non-profile-safe --agent-name requires --profile-name-suffix or an explicit --profile-name\n");
        return 1;
    }
    let installContext;
    try {
    ...
    flags.profileName = resolveBootstrapProfileName(flags.profileName, flags.agentName, flags.profileNameSuffix);
    try {
    ...
    if (flags.resume) {
        ...
        if (!loaded) {
            ...
            if (cleanText(flags.agentName))
                state.agent_name = flags.agentName;
            if (requestedProfileName)
                state.profile_name = flags.profileName;
        }
    }
    else {
        state = makeFreshState(flags);
