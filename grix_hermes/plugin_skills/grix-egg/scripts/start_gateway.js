...
function ensureHermesBinary(hermesCmd) {
    ...
    if ((result.status ?? -1) !== 0) {
        throw new Error(`Hermes CLI '${hermesCmd}' is not available in PATH. ` +
            "Install Hermes first or pass --hermes with an absolute path.");
    }
}
function profilePrefix(hermesCmd, profileName) {
    const normalized = cleanText(profileName);
    if (!normalized || normalized === "default")
        return [hermesCmd];
...
function runDetachedCommand(cmd, env) {
    const [bin, ...rest] = cmd;
    if (!bin)
        throw new Error("runDetachedCommand received empty cmd");
...
function assertGrixProfileConfigured(profileDir) {
...
function assertGatewayOutputHealthy(outputs) {
...
function inspectGatewayLogs(profileDir) {
...
function waitForGrixConnected(profileDir) {
    const deadline = Date.now() + 15000;
    let lastInspection = inspectGatewayLogs(profileDir);
    while (Date.now() < deadline) {
        if (lastInspection.connected)
            return;
        if (lastInspection.unhealthyHint) {
            throw new Error("Hermes gateway started but did not finish loading the grix platform.\n" +
    ...
    throw new Error("Hermes gateway started but did not report a grix connected state within 15 seconds.\n" +
        `log_tail:\n${lastInspection.tail}`);
}
function statusIsRunning(result) {
    if (result.code !== 0)
        return false;
    const combined = summarizeOutput(result).toLowerCase();
    if (!combined)
        return false;
    if (NEGATIVE_STATUS_HINTS.some((hint) => combined.includes(hint)))
        return false;
    if (POSITIVE_STATUS_HINTS.some((hint) => combined.includes(hint)))
        return true;
    return /(\bpid\b|launchd|loaded|plist)/.test(combined);
}
function serviceLooksUnavailable(result) {
    const combined = summarizeOutput(result).toLowerCase();
...
function waitForRunning(statusCmd, env) {
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (token === "--start-subcommand" && next !== undefined) {
            if (next !== "start" && next !== "run")
                throw new Error(`Invalid --start-subcommand: ${next}`);
            flags.startSubcommand = next;
            i += 1;
            continue;
        }
        if (token === "--status-subcommand" && next !== undefined) {
            flags.statusSubcommand = next;
            i += 1;
            continue;
        }
        if (token === "--json") {
...
function main() {
    ...
    try {
        ...
        if (!statusIsRunning(statusAfter)) {
            throw new Error("Hermes gateway did not report a running state after startup.\n" +
