...
function looksLikeGrixHermesRoot(candidate) {
    return (fs.existsSync(path.join(candidate, "bin", "grix-hermes.js")) &&
...
function applyManagementPolicy(currentDisabledSkills, policy) {
    ...
    const nextDisabledSkills = currentDisabledSkills.filter((skillName) => !RESTRICTED_MANAGEMENT_SKILLS.includes(skillName));
...
if (!flags.config) {
    process.stderr.write(`${JSON.stringify({ ok: false, error: "Missing --config." }, null, 2)}\n`);
    process.exit(1);
}
...
const skillsObj = skills && typeof skills === "object" && !Array.isArray(skills)
    ? skills
    : {};
...
if (!["preserve", "main", "restricted"].includes(managementPolicyRaw)) {
    process.stderr.write(`${JSON.stringify({ ok: false, error: `Unsupported --management-policy: ${managementPolicyRaw}` }, null, 2)}\n`);
    process.exit(1);
}
const managementPolicy = managementPolicyRaw;
...
if (requestedGrixWsUrl) {
    ...
    const grix = channels.grix && typeof channels.grix === "object" && !Array.isArray(channels.grix)
        ? channels.grix
        : {};
